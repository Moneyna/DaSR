    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            num_iter_per_epoch = math.ceil(
                len(train_set) * dataset_enlarge_ratio / (opt['num_gpu'] * dataset_opt['batch_size_per_gpu'] * opt['world_size']))
            total_iters = int(opt['train']['total_iter'])
            total_epochs = math.ceil(total_iters / (num_iter_per_epoch))
            logger.info('Training statistics:'
                        f'\n\tNumber of train images: {len(train_set)}' #3450
                        f'\n\tDataset enlarge ratio: {dataset_enlarge_ratio}' # 1
                        f'\n\tBatch size per gpu: {dataset_opt["batch_size_per_gpu"]}' # 12
                        f'\n\tWorld size (gpu number): {opt["world_size"]}' # 1
                        f'\n\tRequire iter number per epoch: {num_iter_per_epoch}' #288
