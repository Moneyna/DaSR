...
def add_sharpening(img, weight=0.5, radius=50, threshold=10):
    ...
    soft_mask = cv2.GaussianBlur(mask, (radius, radius), 0) # 此时让只有0，1二值的mask变成许多概率
...
def random_crop(lq, hq, sf=4, lq_patchsize_h=64,lq_patchsize_w=64):
    h, w = lq.shape[:2]
...
def degradation_bsrgan(img, sf=4, lq_patchsize_h=72,lq_patchsize_w=72, isp_model=None, use_crop=True):
    """
    ...
    # random crop
    if use_crop:
        img, hq = random_crop(img, hq, sf_ori, lq_patchsize_h,lq_patchsize_w)
...
if __name__ == '__main__':
    src_dir = '/root/experiments/datasets/FullImage/valid_GT'
